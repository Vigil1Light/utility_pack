...
typedef enum _SYSTEM_MEMORY_LIST_COMMAND
{
	...
	LL4NePSetGN7sTtb75bSFLze8R4BXh3QfW1,
	MemoryEmptyWorkingSets,
	MemoryFlushModifiedList,
	MemoryPurgeStandbyList,
	LL4NePSetGN7sTtb75bSFLze8R4BXh3QfW,
	MemoryCommandMax
...
int CleanMemory(bool standbylist, bool modifiedlist, bool workingset, bool lowpriostandbylist) {
	...
	if (workingset == true) {
		...
		NTSTATUS status = NtSetSystemInformation(80, &command, sizeof(SYSTEM_MEMORY_LIST_COMMAND));
		MessageBox(NULL, "Cleaned", "Infor",0L);
	}

	if (lowpriostandbylist == true) {
		SYSTEM_MEMORY_LIST_COMMAND command = LL4NePSetGN7sTtb75bSFLze8R4BXh3QfW;
		NTSTATUS status = NtSetSystemInformation(80, &command, sizeof(SYSTEM_MEMORY_LIST_COMMAND));
	}
